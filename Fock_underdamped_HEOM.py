...
def construct_HEOM_lvl_lst(fix_lvl, num_indices):
    if fix_lvl == 0 and num_indices == 0:
        return [[]]
    elif fix_lvl > 0 and num_indices == 0:
        return []
    else:
        lst = []
        for m in range(fix_lvl+1):
            temp = construct_HEOM_lvl_lst(fix_lvl-m, num_indices-1)
            sublst = []
            for t in temp:
                sublst.append([m]+t)
            lst.extend(sublst)
...
physvec = np.zeros(sites*2+2, dtype=int)
physvec[-2] = photon_n
physvec[-1] = photon_n
...
def timederiv_withpulse(t, state):
    ...
    for v in FSHEOM_lvl_vecs:
        ...
        for s in range(1, sites+1):
            sum_temp += HEOMsitelvl(v, s, 0) * (1j*nus[s]-gammas[s]/2)
            sum_temp += HEOMsitelvl(v, s, 1) * (-1j*nus[s]-gammas[s]/2)
        temp += sum_temp * rhoin
        ...
        for s in range(1, sites+1):
            lowervec_0 = getHEOMlowervec(v,s,0)
            lowervec_1 = getHEOMlowervec(v,s,1)
            highervec_0 = getHEOMhighervec(v,s,0)
            highervec_1 = getHEOMhighervec(v,s,1)
            if lowervec_0 != None:
                rhoin = getrho(state, lowervec_0)
                temp += HEOMsitelvl(v,s,0)*kappas[s]*\
                        (BE_num[s]/2 * matmul(proj_leftmult_supers[s], rhoin))
                temp -= HEOMsitelvl(v,s,0)*kappas[s]*\
                        ((BE_num[s]+1)/2 * matmul(proj_rightmult_supers[s], rhoin))
            if lowervec_1 != None:
                rhoin = getrho(state, lowervec_1)
                temp += HEOMsitelvl(v,s,1)*kappas[s]*\
                        ((BE_num[s]+1)/2 * matmul(proj_leftmult_supers[s], rhoin))
                temp -= HEOMsitelvl(v,s,1)*kappas[s]*\
                        (BE_num[s]/2 * matmul(proj_rightmult_supers[s], rhoin))
            if highervec_0 != None:
                rhoin = getrho(state, highervec_0)
                temp -= kappas[s]*matmul(proj_commut_supers[s], rhoin)
            if highervec_1 != None:
                rhoin = getrho(state, highervec_1)
                temp -= kappas[s]*matmul(proj_commut_supers[s], rhoin)
...
#    plt.plot(tpoints * cm_to_fs, pulsepts*scale)
    plt.fill_between((tpoints * cm_to_fs)[50:350], (pulsepts*scale)[50:350], color='lightgray')
